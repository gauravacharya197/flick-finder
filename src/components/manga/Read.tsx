...
const ReadManga = ({ params }: ReadMangaPageProps) => {
  ...
  const seoTitle = manga ? `Read ${manga.title} Chapter ${chapterNumber}${chapterTitle ? ` - ${chapterTitle}` : ''} Online` : 'Read Manga Online';
  const seoDescription = manga ? `Read ${manga.title} Chapter ${chapterNumber} online for free. ${manga.description ? manga.description.substring(0, 120) + '...' : `Enjoy reading ${manga.title} manga with high-quality images.`}` : 'Read manga online for free with high-quality images';
  const seoKeywords = manga ? `${manga.title}, manga, chapter ${chapterNumber}, read online, ${manga.genres?.join(', ') || 'manga genres'}` : 'manga, read online, free manga';
  ...
  return (
    <>
      ...
      <Head>
        ...
        {manga.coverImage && (
          <meta property="og:image" content={`${baseUrl}api/File/image?url=${manga.coverImage}`} />
        )}
        <meta property="og:url" content={`${typeof window !== 'undefined' ? window.location.href : ''}`} />
        <meta property="og:site_name" content="Manga Reader" />
        ...
        {manga.coverImage && (
          <meta name="twitter:image" content={`${baseUrl}api/File/image?url=${manga.coverImage}`} />
        )}
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "@type": "ComicSeries",
            "name": manga.title,
            "description": manga.description || seoDescription,
            "author": {
              "@type": "Person",
              "name": manga.author || "Unknown"
            },
            "genre": manga.genres || [],
            "datePublished": manga.releaseDate,
            "inLanguage": "en",
            "url": typeof window !== 'undefined' ? window.location.href : '',
            "image": manga.coverImage ? `${baseUrl}api/File/image?url=${manga.coverImage}` : '',
            "publisher": {
              "@type": "Organization",
              "name": "Manga Reader"
            }
      ...
      <div className="sr-only">
        <h1>Read {manga.title} Chapter {chapterNumber} Online</h1>
        <p>
          Read {manga.title} Chapter {chapterNumber} online for free. 
          {manga.description && ` ${manga.description.substring(0, 200)}...`}
        </p>
        <div>
          <span>Genres: {manga.genres?.join(', ')}</span>
          <span>Status: {manga.status}</span>
          <span>Release Date: {manga.releaseDate ? new Date(manga.releaseDate).toLocaleDateString() : 'Unknown'}</span>
        </div>
      </div>
      ...
      <div className="flex h-screen bg-background text-gray-200 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full relative">
          ...
          {/* Reader content */}
          <main className="flex-1 overflow-y-auto bg-[#111827]" role="main">
            {pagesLoading ? (
        ...
        {sidebarOpen && (
          <aside className="fixed md:relative w-[100%] md:w-60 h-full bg-[#111827] border-l border-[rgb(31,41,55)] flex flex-col overflow-hidden transition-all duration-300 z-40 left-0" role="complementary">
            ...
            <div className="p-2 border-b border-[rgb(31,41,55)]">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-md font-bold line-clamp-2">{manga.title}</h2>
            ...
            <nav className="flex-1 overflow-y-auto" role="navigation" aria-label="Chapter list">
              {chaptersLoading ? (
                <div className="p-2">
                  <Skeleton rows={5} />
                </div>
                  ...
                  {filteredChapters.map((chapter) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={`Read Chapter ${chNum}${chTitle !== `Chapter ${chNum}` ? ` - ${chTitle}` : ''}`}
                        aria-current={selectedChapter === chapter.id ? 'page' : undefined}
                      >
                        <div className="flex flex-col">
